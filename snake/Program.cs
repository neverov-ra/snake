...
namespace snake
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			p1 = p2; //Присвоение адреса, а не значений. В значения по этому адресу записываем значения далее
			p2.x = 8;
			p2.y = 8;
			Console.WriteLine("p1 = p2. p1.x = " + p1.x + ", p1.y = " + p1.y + "; p2.x = " + p2.x + ", p2.y = " + p2.y);

			p1 = new Point(1, 3, '*');
			Update(p1); //Меняем адреса без изменения значений... Но Update не работает в том виде, в котором он описан в задании (ошибка без параметров)
			Console.WriteLine("Call Move. p1.x = " + p1.x + ", p1.y = " + p1.y);
